...
export function guidanceTaskPrompt(hasActivePlan = false, hasKnowledge = false, hasSkeleton = false, minBeats = 0) {
    return [
        ...
        ...(hasKnowledge ? ['给了「知识库材料」小节时：那是用户自建清单的候选素材（反模型偏好用——用户用清单把候选集合整个换掉，替换掉你自己的常见偏好，不是给你参考）。无论全量还是抽样分组：规划凡涉及该清单领域的内容，必须从该分组的条目里选用、不得自拟同类；抽样分组只发了随机一小把，这一把就是该领域本次的全部候选面。选用的素材要自然融入规划，保持条目的核心特征（地点是酒吧就按酒吧写，不擅自改成餐厅），不生硬罗列、不逐条复述。唯一例外：用户构思或修改意见里点名了该领域的具体内容时，以点名为准、不硬换成清单条目，并在 plan.risks 里如实说明因点名未用清单。本次规划确实不涉及该领域时可以一条不用。并在 plan.knowledgeUsed 里如实报出本次选用条目的编号（小节里【编号 x-xx】的写法），一条没用给空数组。'] : []),
...
// （时间顺序/事实一致性/user 不可编排/点名要求 system 里都在、走缓存价），只留对账要点
// （①～④常驻；⑤知识库口径只在带了知识材料时追加，第十五轮——第一遍按偏好自拟选材时
// 第二遍原「选材不动」条款永远不纠这类问题）；「覆盖上文的生成任务与输出要求」一句负责
// 压过 system 末尾的输出结构（checks 不再输出）
export function alignTailPrompt(hasKnowledge = false) {
    return [
        ...
        ...(hasKnowledge ? ['⑤知识库口径：重读「知识库材料」小节——凡规划涉及某清单领域的内容，选材必须来自该清单的条目、不得自拟同类（保持条目核心特征）；第一遍自拟了的换成小节条目，并同步改报 plan.knowledgeUsed；用户构思或修改意见里点名了该领域具体内容的以点名为准、不换。'] : []),
        ...(hasKnowledge
            ? ['只改违反上述各项的地方，其余原样保留——这是校对不是重写：走向、节点结构不动；条目选材只在违反③点名要求或⑤知识库口径时才更换，换了同步改报 plan.knowledgeUsed，改完仍是同一版规划。每处改动在 fixes 里逐条报出（改了哪里、从什么改成什么、依据哪条要求）；草稿本就全对就原样返回、fixes 给空数组。本次不输出 checks（第一遍的检查沿用）。']
            : ['只改违反上述各项的地方，其余原样保留——这是校对不是重写：走向、节点结构不动；条目选材只在违反③点名要求时才更换，改完仍是同一版规划。每处改动在 fixes 里逐条报出（改了哪里、从什么改成什么、依据哪条要求）；草稿本就全对就原样返回、fixes 给空数组。本次不输出 checks（第一遍的检查沿用）。']),
        '只输出一个符合如下结构的 JSON 对象，不要输出 JSON 以外的任何文字：',
        `{
 ...
 */
// 供规划分析与检查报告共用的材料小节（第十三轮排序：稳定在前、开头会变的垫底）：
// 角色摘要 / 自选世界书 / 游戏玩法 / 记忆表格 / 进行中剧情 / 路人反应 / 历史摘要 / 检索命中 / 最近对话。
