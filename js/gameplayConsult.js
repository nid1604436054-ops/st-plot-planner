...
// 材料刻意轻量——不复用向导第 1 步的整套材料面板：角色摘要/最近对话/世界书自选（向导
// 第 1 步勾选，第四十三轮起不再自动检索）是 materialSections 的固定底座，调用方只按需追加进行中剧情与
...
const CONSULT_SYSTEM_PROMPT = '你是文字角色扮演的玩法设计师。用户会给你一条玩法的「大概思路」与相关材料（角色设定摘要、最近对话、可能自选的世界书条目，可能还有进行中剧情与现有玩法），你把思路扩写成一份完整、可直接执行的玩法规则，给主对话模型在角色扮演中执行。要求：'
 ...
 * 底座材料固定（角色摘要/最近对话/世界书自选——第四十三轮起只带向导第 1 步勾选的条目，
 * 不再自动检索命中），追加项由调用方勾选：activePlan 传空 = 不带进行中剧情，storageItems
 * 传空数组 = 不带现有玩法。
...
export async function generateGameplayDraft({ idea = '', activePlan = '', storageItems = [] } = {}) {
    const { parts } = materialSections({
        ...
        lorePicks: currentLorePicks(),   // 第四十三轮：世界书只带向导第 1 步的勾选（含常驻），自动检索撤出
        headers: {
