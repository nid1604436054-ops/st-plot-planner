...
const CARD_SYSTEM_PROMPT = '你是文字角色扮演的「路人反应校准器」。用户会给你最近对话与相关材料（角色设定、世界书、既往记忆、玩法规则、进行中剧情），'
    ...
    + '不是传播过程的场面描写。'
    + '四、不写 user：反应是路人与世界的，不得替 user 安排动作、台词、态度或心理——user 怎么回应由用户自己写。'
 ...
 * 材料与剧情规划第 1 步共用同一批：materials 即向导 wizardMaterials() 的返回
 * （记忆表格档位/标签/最新行窗口 + 玩法勾选），世界书沿用本对话启用书单；
...
export async function generateReactionCard({ note = '', materials = {}, activePlan = '' } = {}) {
    ...
    // 按「将要且一定会发生」对待，反应卡围绕它出——不是可有可无的参考材料
    const imported = (materials.importedUnits ?? [])
        .map(u => String(u?.text ?? '').trim()).filter(Boolean);
    const user = [
        ...parts,
        ...(imported.length ? ['## 导入单元（用户指定的既定方向：这件事将要且一定会发生，本卡围绕它出）', imported.join('\n\n')] : []),
        '## 任务',
        ...(imported.length
