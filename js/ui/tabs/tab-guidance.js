...
export function resetGuidance() {
    ...
    Object.assign(run, {
        note: '', gpIds: null, kbIds: [], kbListIds: null, result: null, raw: '', hits: 0, planText: '', reviseNote: '', hadActive: false,
...
// 知识库抓取载荷（§6.9）：抓到的条目 id → 发送载荷，只保留「仍在第 1 步勾选中的清单」名下的
// （取消勾选＝这张清单本次不带；条目/清单被删的 id 由 payloadFromIds 静默丢弃）
function wizardKnowledgePayload() {
    const checked = new Set(run.kbListIds ?? []);
    return payloadFromIds(run.kbIds ?? []).filter(p => checked.has(p.list.id));
}
    ...
    <div class="pp-section">
        ...
        <div class="pp-gd-selp">
            ${kbLists.map(l => {
                ...
                return `<label title="勾选后这张清单参与抓取（点下方「知识库抓取」抓条目，抓定未踢的随材料发送）"><input type="checkbox" data-c1k="${escapeHtml(l.id)}" ${(run.kbListIds ?? []).includes(l.id) ? 'checked' : ''}/> ${escapeHtml(l.name)}（${l.entries.length} 条${coolN ? ` · ${coolN} 条冷却中` : ''}）</label>`;
            }).join('')}
        </div>` : ''}
        <div class="pp-btn-row">
            ...
            <span id="pp_gd_kb_panel" class="menu_button" title="知识库抓取（悬浮面板，与随机事件/路人反应同款交互）：勾选中的每张清单各随机抓一小把（条数在「设置 → 知识库」，默认 5、纯随机不按语境过滤、冷却中的条目自动跳过），抓到的条目在面板里看得见——单条可踢、每张清单可整把重抓；抓定未踢的随材料进第 2 步确认页。清单与条目在「知识库」页签管理">知识库抓取</span>
        </div>
...
function renderReady(container, main) {
    ...
    const ut = wizardUnitTexts();
    // 知识库细账：抓定未踢的条目按清单分组点名（助手默认：随材料进确认页，与单元/玩法同一核对口径）
    ...
    <div class="pp-section">
        ...
        <div class="pp-gd-stat" title="第 1 步「知识库抓取」面板里抓定未踢的条目（按清单分组，编号＝清单号-条目号），随材料发送——规划从中选用素材，选用的进冷却">知识库：${kbDesc || '无'}</div>
...
// 首开自动给「勾选中且还没抓过」的清单各抓一把；面板里抓到的条目看得见，
// 单条可踢（不补抓）、每张清单可整把重抓；抓定未踢的随材料进第 2 步确认页。
...
function openKbPanel(onChange) {
    ...
    const render = () => {
        ...
        body.innerHTML = checked.map(list => {
            const payload = payloadFromIds(grabbedOf(list)).filter(p => p.list.id === list.id);
            const available = list.entries.filter(e => !(Number(e.cooldown) > 0)).length;
            const cooling = list.entries.length - available;
            return `
            <div class="pp-gd-ughead">
                <label class="pp-label">${escapeHtml(list.name)}（已抓 ${payload.length} · 可用 ${available} 条${cooling ? ` · ${cooling} 条冷却中` : ''}）</label>
                <span class="menu_button" data-kbregrab="${escapeHtml(list.id)}" title="这张清单整把重抓：丢弃当前抓到的，重新随机抓一批">重抓</span>
            </div>
            ${payload.map(({ listPos, entry }) => `
            <div class="pp-kb-erow" title="${escapeHtml(entryText(list, entry))}">
                <span class="pp-muted pp-kb-ecode">${listPos}-${escapeHtml(entry.code)}</span>
                <span class="pp-kb-ebody">${escapeHtml(entryText(list, entry) || '（空条目）')}</span>
                <span class="menu_button" data-kbkick="${escapeHtml(entry.id)}" title="踢掉这条：不随本次分析发送、不补抓（要换血点整把「重抓」）">踢</span>
            </div>`).join('') || '<div class="pp-muted">这张清单还没抓到条目（点「重抓」或回「知识库」页签加条目）</div>'}`;
        }).join('') + `
        <div class="pp-muted" style="margin-top:6px" title="纯随机、无语境过滤（设计定稿）；条数与冷却次数在「设置 → 知识库」">抓取＝每清单纯随机 ${cfg.grabCount} 条，冷却中的条目自动跳过；踢掉不补抓。抓定未踢的条目随材料进第 2 步确认页，规划选用的进冷却</div>`;
...
async function startAnalyze(container, { revise = false } = {}) {
    ...
    try {
        ...
        const kbPayload = wizardKnowledgePayload();   // 知识库抓定未踢的条目（§6.9）：只进本次规划向导
...
function renderResult(container, main) {
    ...
    main.querySelector('#pp_gd_discard').addEventListener('click', () => {
        Object.assign(run, { result: null, raw: '', hits: 0, planText: '', reviseNote: '', hadActive: false, research: null, kbIds: [] });
        step = 'collect';
