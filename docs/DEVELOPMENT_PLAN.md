...
| 2026-08-24 | 记忆表格镜像「状态」列拆除（应用户三轮反馈确认指的就是「标签」右侧那两字列头）：badgesOf 与四态徽标（新/已改/原表已更新/原表已删）、表头 `<th>状态</th>`、两处 pp-grid-statecell 单元格、.pp-mem-badge 系列与 statecell 样式、空行 colspan +4→+3 全部移除。数据层标记不动——srcUpdated 仍驱动编辑态「采纳原表」按钮（补悬浮说明顶替原徽标的告知职能）、srcGone/seen 仍支撑防复活与新行绿色高亮+「全部标为已读」，只是不再有可见徽标列 |
