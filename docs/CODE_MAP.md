...
| listener.js | ~1240 | 2.0 监听引擎：单位/轻量双模式、两套提示词（块序＝前缀缓存口径：稳定段前置〔说明/单位全文/自选条目/规则/契约〕、节点状态其后、大块楼层再后、三个逐轮重写的小块垫底——第三十四轮重申并写死「楼层在大块材料里最后」的理由：前缀缓存按最长公共前缀计价，楼层排到指导/检索/附加材料之后会让整段楼层每轮重算〕，第二十七轮立、第三十四轮补自选块〕）、判定落账、排队闸、宿主接线、挂载规则（第三十一轮：同 id 再挂不造重复副本——退位槽旧副本作废、活动槽就地换新）、**指导随主人作废制**（第三十二轮：换主人的挂/卸/接回成功即清注入槽＋立 guideVoidReason 作废标记，关总开关/切聊天补标，三轮落账自动解除；判定在途换主人＝整轮作废不落账——runListenerRound 开头记 roundOwnerId、判完对不上就丢，**catch 失败路径同款对账**〔第三十三轮补漏〕）、**回归判定**（第三十三轮：runReentryRound——重挂有进度的长线章对账「走到哪、偏没偏」，五章窗口材料由 longform 侧 scheduleReentryFor 备好传入、buildReentryPrompt/normalizeReentryReport/applyReentryOutcome 三纯函数；只出报告不出指导、不写注入槽、不共例行轮的账〔轮次/指导线/失败计数〕；只进不退批量补点亮、偏大亮红点；与例行轮共用 running 锁、来时在途则等待）、**材料透明化**（lastPromptText 内存留底最近一次提示词全文＋每轮 materials 材料清单小账）、**材料线重做**（第三十四轮：角色卡摘要整条退役〔assembleCharSummary 删〕；世界书自选 assembleLorePicks——勾选键存监听聊天块 state.lorePicks、resolveLorePicks 复用、整条不截断进稳定段、scanLorebooks 传 excludeKeys 让位去重；楼层范围 limitFloors——floorLimit 0=全量/N=最近 N 层角色楼＋其间用户消息、绝对楼层号不变，例行与回归同口径）、**记忆表格归稳定段**（第三十五轮：assembleExtra 改 assembleMemory、独立块排自选条目后、删尾巴附加材料块——记忆表大而少变〔用户实测 4 万字〕，排楼层后每轮跟丢缓存整块全价；楼层后只剩上一轮指导/检索命中两小块；块序通则＝按「大而少变在前、逐轮整写在后」排，不按材料类别贴标签） | runListenerRound、runReentryRound、initListener、listenerProvider、buildUnitPrompt、buildLightPrompt、buildReentryPrompt、createSendGate、lastListenerPrompt、limitFloors |
...
| 监听材料 | listener.js 对应分区＋longform.js 备料 | 例行轮＝章文本全文＋节点状态＋**世界书自选**（监听页勾选器、整条不截断、存监听聊天块）＋**记忆表格**（第三十五轮起排稳定段——大而少变）＋楼层原文（floorLimit 0=全量/N=最近 N 层）＋上一轮指导＋世界书检索命中（与自选去重、自选优先；楼层后只留它与上一轮指导两小块）；回归判定＝五章窗口（longform 备料）替换单位全文、无上一轮指导块；角色卡摘要已撤（第三十四轮——角色资料走世界书自选）；材料清单随每轮留痕落小账（第三十三轮立、三十四改自选口径） |
...
- **离线测试台**：%TEMP%\pp-re-test（第十六轮重建的精简台，第十八轮 61 项、第十九轮扩至 72 项、第二十轮扩至 87 项、第二十四轮 104 项、第二十五轮 113 项、第二十六轮 121 项、第二十七轮 136 项、第二十八轮 146 项、第二十九轮 149 项、第三十轮 166 项、第三十一轮 180 项、第三十二轮 199 项、第三十三轮 233 项、第三十四轮 254 项、第三十五轮 255 项——改动 js 后记得把工作区文件拷进测试台再跑，测试台 import 的是自己的副本；jsonResponse 桩的 text 字段返回真 JSON 正文——非流式路径读 text 再本地 parse，桩与真实 Response 对齐；setExtensionPrompt 替身全量留痕进 globalThis.__slotWrites、document 替身带 dispatchEvent〔第三十二轮起断言注入槽用〕；held-fetch 竞态断言注意 listenerAttempt 会重试两发——「先挂起再拒」的桩第二发要立即拒，否则吊 90 秒超时〔第三十三轮教训〕）；%TEMP% 会被系统清理——重要断言随轮次记进交付记录，丢了照记录重建（搭法在记忆 offline-testbed-technique）。
