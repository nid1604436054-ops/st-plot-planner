...
- `chatdata`（按聊天身份）：记忆镜像/剧情档案/向导勾选 picks/单元池/监听留痕与状态/向导快照（wizard 块）/近期草稿骨架。
...
- **离线测试台**：%TEMP%\pp-re-test（第十六轮重建的精简台）；%TEMP% 会被系统清理——重要断言随轮次记进交付记录，丢了照记录重建（搭法在记忆 offline-testbed-technique）。
