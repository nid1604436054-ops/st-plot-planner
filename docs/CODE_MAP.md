...
| materials.js | ~125 | 材料小节拼装（规划/检查/事件/反应共用——预览与真实调用同一拼法；第四十三轮：检索命中小节撤出、自选小节随 lorePicks 走；currentLorePicks＝向导勾选〔常驻∪手选、loreV=2 门槛〕的唯一读取口，三工具/检查报告/混合重编同源） | materialSections、currentLorePicks |
...
| 文件 | 行数 | 职责 | 关键导出 |
|---|---|---|---|
| planner.js | ~680 | 规划分析（两遍调用编排）/检查报告/联网研究/规划系统提示词（第四十三轮：检查报告世界书改带向导勾选 runStoryReview lorePicks 参数、collectStats 撤命中数、actionRefSection 改导出且只看自选——混合重编也吃） | runPlotGuidance、runStoryReview、buildGuidanceMessages、guidanceSystemPrompt、alignTailPrompt、actionRefSection、collectStats |
...
| randomEvents.js | ~310 | M3 随机事件三层（维度/条目/掷骰管线）＋三路生成；第四十三轮：生成材料世界书改带向导勾选〔materials.lorePicks ?? currentLorePicks()〕，掷骰的事件关键词窗口仍吃 settings.retrieval.scanDepth（设置页「事件关键词扫描层数」） | 各生成入口 |
| reactions.js | ~165 | 路人反应校准卡生成（第四十三轮：世界书改带向导勾选） | 生成入口 |
